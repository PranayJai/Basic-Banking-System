...
Account.prototype.deposit = function(amount) {
    if (this._isPositive(amount)) {
        this.value += amount;
        console.info(`Deposit: ${this.name} new value is ${this.value}`);
        return true;
    }
    return false;
}
Account.prototype.withdraw = function(amount) {
    if (this._isAllowed(amount)) {
        this.value -= amount;
        console.info(`Withdraw: ${this.name} new value is ${this.value}`);
        return true;
    }
    return false;
}
...
Account.prototype._isAllowed = function(amount) {
    ...
    const isAllowed = this.value - amount >= 0;
    if (!isAllowed) {
...
output.innerText += `before:  a: ${a.value}, b: ${b.value}\n`;

a.transfer(100, b);

output.innerText += `after:  a: ${a.value}, b: ${b.value}\n`
